...
def Roulette_wheel(I, fitnesses):
        ...
        #X is the normalized vector of the cummulative fitnesses
...
def fitness(i, genetics, E, t):
    if E==0: #just one env A, no changes
...
def algorithm(I, genetics, E, t, lam, mut, seed, N):
    ...
    for index, t_point in enumerate(t):
        ...
        for j in I: #new individuals
            ...
            mother=parents[0][0]
            father=parents[0][1]
            a_pj=(1-lam)*rng.binomial(genetics[mother][0], 1/2)+ lam*rng.binomial(genetics[father][0], 1/2)
            b_pj=(1-lam)*rng.binomial(genetics[mother][1], 1/2)+ lam*rng.binomial(genetics[father][1], 1/2)
            m_pj=(1-lam)*rng.binomial(genetics[mother][2], 1/2)+ lam*rng.binomial(genetics[father][2], 1/2)
...
#run the simulations
seeds=[666, 200, 500, 0, 1 ,2,3,4,5,6] #10 random seeds in total
Es=[0,50,70,100,150] #time point when the change occurs
Ns=[10,50,100,200] #bottleneck size, oDNA population size
lams=[0, 0.01, 0.05, 0.25] #leakage
muts=[10**-6, 10**-3, 10**-2] #mutation
I=[i for i in range (0,100)] # 100 individualsper generation, population size
t=[i for i in range (0,500)] # 500 generations
#each individual starts with a=b=N/2, and no mutant load m=0
for seed in seeds:
    ...
    for N in Ns:
        for E in Es:
            for lam in lams:
                for mut in muts:
                    algorithm(I, [[N/2,N/2,0] for i in I], E, t, lam, mut, seed, N)
    print(seed)
