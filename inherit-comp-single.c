...
#define NGEN 501        // number of generations in simulation
...
int main(int argc, char *argv[])
{
  ...
  if(argc != 8) {
    printf("Please specify Npop, fitness scale, heteroplasmy penalty, templating rate, rel int fitness of B allele, rel int fitness of C allele, random inheritance\n");
    exit(0);
  }
  NPOP = atoi(argv[1]);
  scale = atof(argv[2]);
  ...
  FITNESSB = atof(argv[5]);
  FITNESSC = atof(argv[6]);
  RANDOM = atoi(argv[7]);
  ...
  fp = fopen(fstr, "w");
  fprintf(fp, "Npop,ICs,scale,penalty,det.reamp,det.leak,template,fit.b,fit.c,,env,nDNA,mu,DUI,leakage,expt,t,i,a,b,c,f\n");
#endif

#ifdef _MEANOUTPUT
  sprintf(fstr, "inherit-comp-single-%i-%.3f-%.3f-%.5f-%.3f-%.3f-%i.csv", NPOP, scale, penalty, TEMPLATE, FITNESSB, FITNESSC, RANDOM);
  fpm = fopen(fstr, "w");
  fprintf(fpm, "env,mu,Npop,scale,penalty,template,fit.b,fit.c,expt,t,curr.env,mean.f,var.f,mean.h,mean.leak,var.leak,mean.NDNA,var.NDNA\n");
#endif
  ...
  fpc = fopen(fstr, "w");
  fprintf(fpc, "Npop,ICs,scale,penalty,det.reamp,det.leak,template,fit.b,fit.c,env,nDNA,mu,DUI,leakage,expt,end.mean.f,end.mean.h,window.mean.f\n");
#endif

  DUI = 0;
  for(env = 1; env <= 128; env *= 2)
    {
      for(MU = 0; MU <= 1e-2; MU *= 5)
	{
          printf("%i %f\n", env, MU);
	  ...
	  for(expt = 0; expt < NSAMP; expt++)
	      ...
	      for(t = 0; t < NGEN; t++)
		{
...
#ifdef _MEANOUTPUT
		  if(expt < 10 || t % 100 == 0) {
		  fprintf(fpm, "%i,%f,   %i,%f,%f,  %f,%f,%f,  %i,%i,%i,  %f,%f,%f,  %f,%f,%f,%f\n",
			  env, MU,
			  NPOP, scale, penalty,
			  TEMPLATE, FITNESSB, FITNESSC,
			  expt, t, (t/env)%2, 
			  meanf, varf, meanh,
		  ...
		  for(i = 0; i < NPOP; i++)
			...
			{
			  fprintf(fp, "%i,%i,%f,%f,%i,%i,%f,%f,%f,%i,%i,%f,%i,%f,%i,%i,%i,%i,%i,%i,%f\n", NPOP, ICs, scale,penalty, DET_REAMP, DET_LEAK, TEMPLATE, FITNESSB, FITNESSC, env, NDNA, MU, DUI, LEAKAGE, expt, t, i, I[i].a, I[i].b, I[i].c, f[i]);
			}
		    }
#endif
		  ...
		  for(i = 0; i < NPOP; i++)
			...
			newI[i].leakage = I[mum].leakage;
		      }
		      if(newI[i].leakage < 0) newI[i].leakage = 0;
		      if(newI[i].leakage > 0.5) newI[i].leakage = 0.5;
		      if(RANDOM) {
  		        newI[i].NDNA = pow(10, (log10(I[mum].NDNA) + log10(I[dad].NDNA))/2. + gsl_ran_gaussian(0.25));
		      } else {
			newI[i].NDNA = I[mum].NDNA;
		      }
