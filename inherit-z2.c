...
#ifdef _FULLOUTPUT
  sprintf(fstr, "inherit-z2-full-out-%i-%.3f-%.3f.csv", env, scale, penalty);
  fp = fopen(fstr, "w");
  fprintf(fp, "scale,penalty,env,nDNA,mu,DUI,leakage,expt,t,i,a,b,c,f\n");
#endif

#ifdef _MEANOUTPUT
  sprintf(fstr, "inherit-z2-mean-out-%i-%.3f-%.3f.csv", env, scale, penalty);
  fpm = fopen(fstr, "w");
  ...
  //  for(env = 0; env <= 6; env++)
  {
	...
	for(MU = 0; MU <= 0.011; MU += 0.01)
	{
	  ...
	  // for(DUI = 0; DUI <= 1; DUI++)
	  {
	    ...
	    for(LEAKAGE = 0; LEAKAGE <= 0.5; LEAKAGE *= 1.5)
	      {
...
#ifdef _MEANOUTPUT
			if(env == 0) reporttime = 999;
			else reporttime = ((int)(1000./env))*env - 1;
