...
#ifdef _FULLOUTPUT
  sprintf(fstr, "inherit-baseline-full-out-%i-%i-%.3f-%.3f.csv", NPOP, env, scale, penalty);
  fp = fopen(fstr, "w");
  fprintf(fp, "Npop,scale,penalty,env,nDNA,mu,DUI,leakage,expt,t,i,a,b,c,f\n");
#endif

#ifdef _MEANOUTPUT
  sprintf(fstr, "inherit-baseline-mean-out-%i-%i-%.3f-%.3f.csv", NPOP, env, scale, penalty);
  fpm = fopen(fstr, "w");
