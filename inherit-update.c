...
int main(int argc, char *argv[])
{
  ...
  if(argc != 10) {
    printf("Please specify Npop, initial conditions, environmental change period, fitness scale, heteroplasmy penalty, deterministic reamp, deterministic leakage, templating rate, rel int fitness of B allele\n");
    exit(0);
  }
  NPOP = atoi(argv[1]);
  ICs = atoi(argv[2]);
...
#ifdef _MEANOUTPUT
  sprintf(fstr, "inherit-template-mean-out-%i-%i-%i-%.3f-%.3f-%i-%i-%.5f-%.2f.csv", NPOP, ICs, env, scale, penalty, DET_REAMP, DET_LEAK, TEMPLATE, FITDIFF);
  fpm = fopen(fstr, "w");
  fprintf(fpm, "Npop,ICs,scale,penalty,det.reamp,det.leak,template,fitdiff,env,nDNA,mu,DUI,leakage,expt,t,mean.f,var.f,mean.h\n");
#endif
...
#ifdef _MEANOUTPUT
			    if(expt < 4)
			      fprintf(fpm, "%i,%i,%f,%f,%i,%i,%f,%f,%i,%i,%f,%i,%f,%i,%i,%f,%f,%f\n", NPOP, ICs, scale, penalty, DET_REAMP, DET_LEAK, TEMPLATE, FITDIFF, env, NDNA, MU, DUI, LEAKAGE, expt, t, meanf, varf, meanh);
#endif
...
#ifdef _CHANGEOUTPUT
			fprintf(fpc, "%i,%i,%f,%f,%i,%i,%f,%f,%i,%i,%f,%i,%f,%i,%f,%f,%f\n", NPOP, ICs, scale,penalty,DET_REAMP, DET_LEAK, TEMPLATE, FITDIFF, env, NDNA, MU, DUI, LEAKAGE, expt,  meanmean/meannorm, meanmeanh/meannorm, meanw/meanwnorm);
#endif
