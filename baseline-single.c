...
#define MAXPOP 100         // population size
#define NGEN 5000        // number of generations in simulation
...
int main(int argc, char *argv[])
{
  ...
  if(argc != 5) {
    printf("Please specify organismal population size, environmental protocol (0-6), fitness scale, and heteroplasmy penalty\n");
    exit(0);
  }
  NPOP = atoi(argv[1]);
  env = atoi(argv[2]);
...
#ifdef _FULLOUTPUT
  sprintf(fstr, "inherit-baseline-single-full-out-%i-%i-%.3f-%.3f.csv", NPOP, env, scale, penalty);
  fp = fopen(fstr, "w");
  fprintf(fp, "Npop,scale,penalty,env,nDNA,mu,DUI,leakage,expt,t,i,a,b,c,f\n");
#endif

#ifdef _MEANOUTPUT
  sprintf(fstr, "inherit-baseline-single-mean-out-%i-%i-%.3f-%.3f.csv", NPOP, env, scale, penalty);
  fpm = fopen(fstr, "w");
      ...
      //      for(NDNA = 10; NDNA < 200; NDNA *= 1.2)
	{
	  ...
	  for(MU = 0; MU <= 0.01; MU *= 2)
	      ...
	      //	      for(DUI = 0; DUI <= 1; DUI++)
		{
		  ...
		  //  for(LEAKAGE = 0; LEAKAGE <= 0.5; LEAKAGE *= 2)
		    {
